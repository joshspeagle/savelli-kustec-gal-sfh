    ...
    for sim in sim_name_list:
        ...
        if filepath.exists():
            try:
                ...
                sim_data[sim] = {
                    "sfh_raw": (
                        mat_data.get("smallsfhs", None).T
                        if mat_data.get("smallsfhs", None) is not None
                        else None
                    ),  # Transpose like original: (galaxies, time)
                    "times": mat_data.get("smalltime", None),  # Time array
                    "sm": 10
                    ** mat_data.get("logmass", np.array([[]]))[
                        0
                    ],  # Stellar mass (flatten)
                    "ngal": (
                        mat_data["logmass"].shape[1] if "logmass" in mat_data else 0
                    ),  # Number of galaxies
                }
...
def apply_mass_cuts(sim_data, cuts=None):
    ...
    for sim, data in sim_data.items():
        ...
        if sim in cuts and "sm" in data and data["sm"] is not None:
            ...
            print(
                f"{sim}: {np.sum(mask)}/{len(mask)} galaxies after M* > {cuts[sim]:.0e} cut"
            )
        else:
...
def remove_zero_sfhs(sim_data):
    ...
    for sim, data in sim_data.items():
        ...
        non_zero_mask = np.any(
            sfh_raw > 0, axis=1
        )  # Check along time axis (axis=1 now)
        ...
        print(
            f"{sim}: {np.sum(non_zero_mask)}/{len(non_zero_mask)} galaxies after removing zero SFHs"
        )
...
def prepare_umap_data(sim_data, normalize=True, remove_zeros=True):
    ...
    print(
        f"Combined data: {len(combined_sfh)} total galaxies with {combined_sfh.shape[1]} time bins"
    )
