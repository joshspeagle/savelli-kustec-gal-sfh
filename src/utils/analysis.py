...
SFR_axis_label = r"$\mathrm{SFR}\ [\mathrm{M}_\odot\mathrm{yr}^{-1}]$"
sSFR_axis_label = r"$\mathrm{sSFR}\ [\mathrm{yr}^{-1}]$"
...
colors = {
    "EAGLE": "limegreen",
    "Illustris": "deepskyblue",
    "IllustrisTNG": "royalblue",
    "Mufasa": "darkorange",
    "Simba": "gold",
    "SC-SAM": "hotpink",
    "UniverseMachine": "darkorchid",
}
    ...
    for i in range(10):
        for j in range(10):
            if umaps[i, j].shape[0] < 100:
                p_x_s[i * 10 : i * 10 + 10, j * 10 : j * 10 + 10] = np.full(
                    (10, 10), np.nan
                )  # get rid of boxes with too few points
...
def calc_weights_hists(data1, data2=None, weights=None):
    ...
    for t in range(nsnaps):
        ...
        # compute new weights
        if data2 is None:
            W_t, ess_t = calc_weights(
                data1=data1[t], mu1=qt_data1[1], sigma1=(qt_data1[2] - qt_data1[0]) / 2
            )
        else:
            ...
            W_t, ess_t = calc_weights(
                data1=data1[t],
                mu1=qt_data1[1],
                sigma1=(qt_data1[2] - qt_data1[0]) / 2,
                data2=data2[t],
                mu2=qt_data2[1],
                sigma2=(qt_data2[2] - qt_data2[0]) / 2,
            )
